        ...
        try(InputStream inputStream = getClass().getClassLoader().getResourceAsStream(fileLocation);
            ...
            try(InputStreamReader inputStreamReader = new InputStreamReader(inputStream,
                ...
                while ((line = bufferedReader.readLine()) != null) {
                    ...
                    tariffList.add(new Tariff(Integer.parseInt(token[0]), token[1], token[2], Integer.parseInt(token[3]), Integer.parseInt(token[4]), Integer.parseInt(token[5]), Long.parseLong(token[6])));
                }
            }
        } catch (IOException e) {
